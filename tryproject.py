...
music_app = tk.ThemedTk()

music_app.get_themes()
music_app.set_theme("breeze")

music_app.title('Music Player')
music_app.geometry('600x600')
music_app.wm_iconbitmap('music.ico')

status_bar_label = ttk.Label(music_app, text='Welcome To the Music Player', relief=GROOVE, borderwidth=2,
                             font=('Helvetica', 12))
status_bar_label.pack(side=BOTTOM, fill=X)

button_frame = Frame(music_app)
button_frame.pack(side=BOTTOM, fill=X)

length_frame = Frame(music_app)
...
music_player_icon = PhotoImage(file='Media Player.png')
main_frame_music_icon = PhotoImage(file='main_frame_music_1.png')
...
index = 0
playlist_box = Listbox(music_app)
playlist_list = []
...
def nextsong():
    ...
    if playlist_var:
        try:
            ...
            messagebox.showerror('File Not Found', 'Could not find file to play.')
...
def playlist():
    ...
    if not playlist_var:
        ...
        playlist_btnframe = ttk.Frame(music_app)
        playlist_btnframe.pack(side=BOTTOM, fill=BOTH)

        playlist_frame = ttk.Frame(music_app)
        playlist_frame.pack(side=TOP, fill=BOTH)

        playlist_box = Listbox(playlist_frame, height=30, selectbackground='turquoise', relief=GROOVE, borderwidth=3)
...
def playmusic():
    ...
    try:
        ...
        messagebox.showerror('File Not Found', 'Could not find file. ')
...
def stopmusic():
    ...
    if not stop_var:
        ...
        messagebox.showerror('File Not Found', 'Could not find file to stop.')
...
photo_frame = Label(music_app, image=main_frame_music_icon)
photo_frame.pack(side=BOTTOM, fill=BOTH)
...
music_app.config(menu=main_menu)
...
music_app.protocol("WM_DELETE_WINDOW", on_closing)

# Bind Shortcut keys
music_app.bind("<Control-o>", browse_file)
music_app.bind("<Control-Shift-o>", multiple_files)
music_app.bind("<Control-w>", open_folder)
music_app.bind("<Control-q>", exit_command)
